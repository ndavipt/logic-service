# PowerShell script to run a mock scraper server for local development
...
# Create a simple FastAPI app that mocks the scraper service
$mockScraperCode = @'
...
# Write the mock server code to a temporary file
$mockServerPath = ".\mock_scraper_server.py"
$mockScraperCode | Out-File -FilePath $mockServerPath -Encoding utf8

# Make sure uvicorn is installed
pip install uvicorn

# Run the mock server directly with python
Write-Host "Starting mock scraper server on port 8001..." -ForegroundColor Green
python $mockServerPath