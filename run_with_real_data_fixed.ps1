# PowerShell script to run the Logic Service with real data (fixed version)
...
# First run the router fix
Write-Host "Fixing router issues..." -ForegroundColor Yellow
.\fix_router.ps1

# Create a simplified Python script to run with real data
$scriptContent = @"
import asyncio
import httpx
import json
...
# Write the script
$scriptContent | Out-File -FilePath "run_with_real_data_fixed.py" -Encoding utf8

# Run the script to fetch data and start the server
Write-Host "Fetching data from Scraper Service and starting Logic Service..." -ForegroundColor Green
python run_with_real_data_fixed.py